...
public class StartMojo extends AbstractMojo {
     ...
     */
    private boolean _separateConfigurationFiles;
     ...
     */
    private boolean _writeConfIndex;
     ...
     */
    private Properties _hadoopConfiguration;
     ...
     * @readonly
     */
    private List<Artifact> _pluginDependencyArtifacts;
     ...
     * exist.
     *
     * @param file the file to get the parent directory from.
     * @return The parent directory.
     * @throws MojoExecutionException If there is an error getting or creating the parent directory.
     */
    private static File createFileParentDir(final File file) throws MojoExecutionException {
     ...
     *
     * @param stream what to close
     * @throws MojoExecutionException If there is an error closing the stream.
     */
    private static void closeFileOutputStream(final FileOutputStream stream) throws MojoExecutionException {
        if (null != stream) {
            try {
                stream.close();
            }
     ...
     */
    public void setHbaseSiteFile(final File hbaseSiteFile) {
        _hBaseSiteFile = hbaseSiteFile;
    }
     ...
     */
    public void setSeparateConfigurationFiles(final boolean separateConfigurationFiles) {
        _separateConfigurationFiles = separateConfigurationFiles;
    }
     ...
     */
    public void setCoreSiteFile(final File coreSiteFile) {
        _coreSiteFile = coreSiteFile;
    }
     ...
     */
    public void setWriteConfIndex(final boolean writeConfIndex) {
        _writeConfIndex = writeConfIndex;
    }
     ...
     */
    public void setHbaseConfIndex(final File hbaseConfIndex) {
        _hBaseConfIndex = hbaseConfIndex;
    }
     ...
     */
    public void setHadoopConfiguration(final Properties properties) {
        _hadoopConfiguration = properties;
    }
    ...
    public void execute() throws MojoExecutionException {
        ...
        if (null != _hadoopConfiguration) {
            for (final Map.Entry<Object, Object> property : _hadoopConfiguration.entrySet()) {
                final String confKey = property.getKey().toString();
                final String confValue = property.getValue().toString();
        ...
        // Start the cluster.
        try {
            PluginMiniHBaseClusterSingleton.INSTANCE.startAndWaitUntilReady(
                getLog(), _isMapReduceEnabled, conf);
        }
        catch (final IOException e) {
            throw new MojoExecutionException("Unable to start HBase cluster.", e);
        }

        if (_separateConfigurationFiles) {
            ...
            writeSiteFile(conf, _hBaseSiteFile);
        }
     ...
     */
    private void removeHadoopTmpDir(final Configuration conf) {
        final String hadoopTmpPath = conf.get("hadoop.tmp.dir");
        final File hadoopTmp = new File(hadoopTmpPath);
        if (hadoopTmp.exists()) {
     ...
     * @throws MojoExecutionException when the Mojo cannot be done.
     */
    private String getClassPath() throws MojoExecutionException {
        ...
        final String existingClasspath = System.getProperty("java.class.path");
        ...
        try {
            testClasspathComponents = _mavenProject.getTestClasspathElements();
        }
        catch (final DependencyResolutionRequiredException e) {
            throw new MojoExecutionException("Unable to retrieve project test classpath", e);
        }
        for (final Object testClasspathComponent : testClasspathComponents) {
            final String dependency = testClasspathComponent.toString();
        ...
        for (final Artifact artifact : _pluginDependencyArtifacts) {
            final String dependency = artifact.getFile().getPath();
            if (alreadyAddedComponents.contains(dependency)) {
                continue;
            }
            classpath.append(":");
     ...
     */
    private void writeHBaseSiteFile(final Configuration conf) throws MojoExecutionException {
        writeSiteFile(getHBaseOnlyConfiguration(conf), _hBaseSiteFile);
    }
     ...
     * @throws MojoExecutionException If there is an error writing the file.
     */
    private void writeCoreSiteFile(final Configuration conf) throws MojoExecutionException {
        writeSiteFile(getMapReduceOnlyConfiguration(conf), _coreSiteFile);
    }
     ...
     * @throws MojoExecutionException If there is an error writing the file.
     */
    private void writeSiteFile(final Configuration conf, final File siteFile) throws MojoExecutionException {
     ...
     */
    private Configuration getHBaseOnlyConfiguration(final Configuration conf) {
     ...
     * @return A new configuration containing copies of the appropriate configuration variables.
     */
    private Configuration getMapReduceOnlyConfiguration(final Configuration conf) {
     ...
     */
    private Configuration getFilteredConfiguration(final Configuration conf, final boolean hBaseOnly) {
        final Configuration filteredConf = new Configuration(false);
        for (final Map.Entry<String, String> entry : conf) {
            final boolean startsWithHBase = entry.getKey().startsWith("hbase");
    ...
    private void writeConfigurationIndex() throws MojoExecutionException {
        ...
        createFileParentDir(_hBaseConfIndex);

        // Write the file.
        FileOutputStream fileOutputStream = null;
        PrintWriter fileWriter = null;
        try {
            fileOutputStream = new FileOutputStream(_hBaseConfIndex);
            fileWriter = new PrintWriter(fileOutputStream);
            fileWriter.println(_hBaseSiteFile.getPath());
        }
        catch (final IOException e) {
            throw new MojoExecutionException(
                "Unable to write to configuration index file: " + _hBaseConfIndex.getPath(), e);
        }
        finally {
            if (null != fileWriter) {
                fileWriter.close();
        ...
        getLog().info("Wrote " + _hBaseConfIndex.getPath() + ".");
    }
}
